...
local loop = ioloop()
local now = loop.time()
local ten = loop:add_timeout(now + 10, function() print('you shouldn\'t see me') end)
loop:add_timeout(now + 11, function() print('11 secs has passed.') end)
loop:add_timeout(now + 5, function()
  loop:remove_timeout(ten)
  print('5 secs has passed.')
end)
loop:start()
